...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        //set signed in to true just to start
    ...
    func configureAuth() {
        ...
        _authHandle = FIRAuth.auth()?.addStateDidChangeListener { (auth: FIRAuth, user: FIRUser?) in
            ...
            if let activeUser = user {
                //check if app user is curent FIRUser
    ...
    deinit {
        ...
        ref.child("message").removeObserver(withHandle: _refHandle)
    ...
    func sendPhotoMessage(photoData: Data) {
        ...
        // build a path using the user ID and timestamp
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageUrl = message[Constants.MessageFields.imageUrl] {
            ...
            FIRStorage.storage().reference(forURL: imageUrl).data(withMaxSize: INT64_MAX) { (data, error) in
                ...
                // check if the cell is still on screen, if so, fill it
